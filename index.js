...
function kan(inputStr)
{
	...
	else if(inputStr.toLowerCase().match('evnt') != null)
	{
		...
		return rplyArr[eval(RollDice('2d6').toString())-1];	
	}
	//戰場表
	...
	else if(inputStr.toLowerCase().match('rnt') != null)
	{
		...
		'暴走表(2) → 興奮：變的好戰，鬥爭本能完全裸露出來，即使受傷也不在意。需要消耗１Ｄ６【行動力】才能進行迴避判定。',
		'暴走表(3) → 興奮：興奮狀態，會過度出力。自己消費或減少【行動力】時，會多支付１點。',
		'暴走表(4) → 興奮：除了某人以外的一切都不會進入視線，被對那個人的愛迷惑。只能對【感情值】最高的人聲援，若有複數，則無法對任何人聲援。',
		'暴走表(5) → 興奮：自視甚高，變的自己為了不起。進行行為判定時，２Ｄ６的數字在４以下就算大失誤。',
		'暴走表(6) → 興奮：完全失去希望，容易受到各種傷害自己受到傷害時，傷害＋１Ｄ６。',
		];
		return rplyArr[Math.floor((Math.random() * (rplyArr.length)) + 0)];	
	}
	else
